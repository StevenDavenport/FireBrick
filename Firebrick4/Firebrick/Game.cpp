...
{	
	...
	// Instantiate Players
	mPlayers.push_back(std::make_shared<CPlayer>("Sorceress", "sorceress.txt", yn));	
	mPlayers.push_back(std::make_shared<CPlayer>("Wizard", "wizard.txt", yn));
}
...
{							
	...
	// Printing field
	std::cout << "Sorceress : ";
	int sorcFieldSize = mPlayers[0]->GetFieldSize();
	for (size_t i = 0; i < sorcFieldSize; i++)
	{
		weakCard scard = mPlayers[0]->GetCard(i);
		// Make sure the weak pointer hasn't expired
		if (!scard.expired())
		{
			std::cout << scard.lock()->GetName() << " (" << scard.lock()->GetHealth() << ")  ";
		}
	}
	std::cout << std::endl;
	...
	// Printing field
	std::cout << "Wizard : ";		
	int wizardFieldSize = mPlayers[1]->GetFieldSize();
	for (size_t i = 0; i < wizardFieldSize; i++)
	{
		weakCard wcard = mPlayers[1]->GetCard(i);
		// Make sure the weak pointer hasn't expired
		if (!wcard.expired())
		{
			std::cout << wcard.lock()->GetName() << " (" << wcard.lock()->GetHealth() << ")  ";
		}
	}
	std::cout << std::endl;
	std::cout << "--------------" << std::endl;
...
//		mAttackDammage = mFriendlyField[i]->GetAttack();
//
//		// Name of the attacking minion
//		mAttackingMinionName = mFriendlyField[i]->GetName();
//
//		// flag
...
//		{
//			if (mEnemyField[i]->GetSpecialAbility() == wall) 
//			{
//				wallOnField = true;
//				wallIndex = i;
//			}
...
//			// Refresh 
//			mEnemyField = mPlayers[mEnemyIndex]->GetField();
//			mMinionUnderAttack = mEnemyField[wallIndex]->GetName();
//
//			// If minion dies
//			if (mEnemyField[wallIndex]->GetHealth() <= 0)
//			{
//				// Delete from play
...
//			{
//				std::cout << mAttackingMinionName << " attacks " << mMinionUnderAttack << "... ";
//				std::cout << mMinionUnderAttack << " health now : " << mEnemyField[wallIndex]->GetHealth() << std::endl;
//			}
//		}
//		// Attack Player
...
//
//			// Card health
//			mMinionUnderAttackHealth = mEnemyField[mR]->GetHealth();
//
//			// Reduce that cards health
//			mPlayers[mEnemyIndex]->ReduceCardHealth(mR, mAttackDammage);
//
//			// Dammaged minions name
//			mMinionUnderAttack = mEnemyField[mR]->GetName();
//
//			// Refresh
//			mEnemyField = mPlayers[mEnemyIndex]->GetField();
//
//			// Minion dies
//			if (mEnemyField[mR]->GetHealth() <= 0)
//			{
//				// Delete from play
...
//				// Check for trample 
//				if (mAttackDammage > 0 && mFriendlyField[i]->GetSpecialAbility() == trample) 
//				{
//					CastTrample(i);
//				}
//			}
...
//			{
//				std::cout << mAttackingMinionName << " attacks " << mMinionUnderAttack << "... ";
//				std::cout << mMinionUnderAttack << " health now : " << mEnemyField[mR]->GetHealth() << std::endl;
//			}
//		}
//		// If card is a vampire heal it
//		if (mFriendlyField[i]->GetSpecialAbility() == vampire) 
//		{
//			mPlayers[mFriendlyIndex]->IncreaseCardHealth(i, 1);
//			std::cout << mAttackingMinionName << " heals... ";
//			std::cout << mAttackingMinionName << " health now : " << mFriendlyField[i]->GetHealth() << std::endl;
//		}
//		// If card is a leech heal player
//		if (mFriendlyField[i]->GetSpecialAbility() == leech) 
//		{
...
//		std::string enemyMinion = mEnemyField[i]->GetName();
//
//		// Minion has been killed
//		if (mEnemyField[i]->GetHealth() <= 0) 
//		{
//			// Delete from field
...
//			// Output to console
//			std::cout << "Lightning strikes " << enemyMinion << "... ";
//			std::cout << enemyMinion << " health now : " << mEnemyField[i]->GetHealth() << std::endl;
//		}
//	}
...
//			std::string minionHealed = mFriendlyField[mR]->GetName();
//
//			std::cout << "Bless heals " << minionHealed << "... ";
//			std::cout << minionHealed << " health now : " << mFriendlyField[mR]->GetHealth() << std::endl;
//		}
//	}
//}
//
...
//	for (size_t i = 0; i < mFriendlyFieldSize; i++)
//	{
//		if (mFriendlyField[i]->GetSpecialAbility() == horde)
//		{
//			hordeCount++;
//		}
//	}
...
//	for (size_t i = 0; i < mFriendlyFieldSize; i++)
//	{
//		if (mFriendlyField[i]->GetSpecialAbility() == horde && hordeCount > 1)
//		{
//			mPlayers[mFriendlyIndex]->IncreaseCardAttack(i, 1);
//			std::cout << mFriendlyField[i]->GetName() << " increases it's attack by one" << std::endl;
//		}
//	}
//}
//
...
//		std::cout << "Sword increases " << mFriendlyField[mR]->GetName() << "'s attack by by 2" << std::endl;
//	}
...
//		std::cout << "Armour increases " << mFriendlyField[mR]->GetName() << "'s protection by 2" << std::endl;
//	}
//	// If there are no cards on the friendly field, increase the mPlayers health and output it
//	else
//	{
